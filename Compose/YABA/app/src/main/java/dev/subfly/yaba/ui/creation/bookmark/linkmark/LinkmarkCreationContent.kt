...
import dev.subfly.yaba.core.state.creation.linkmark.LinkmarkCreationEvent
import dev.subfly.yaba.core.state.creation.linkmark.LinkmarkCreationToastMessages
...
fun LinkmarkCreationContent(bookmarkId: String?, initialUrl: String? = null) {
    ...
    val linkmarkToastMessages =
            LinkmarkCreationToastMessages(
                    ...
                    invalidUrl = stringResource(R.string.url_error_text),
                    unableToUnfurl = stringResource(R.string.unfurl_error_text),
                    genericUnfurlError = stringResource(R.string.generic_unfurl_error_text),
                    acceptLabel = stringResource(R.string.ok),
            )

    val converterInput by
            remember(state.converterHtml) {
                derivedStateOf {
    ...
    LaunchedEffect(bookmarkId, initialUrl) {
        vm.onEvent(
                LinkmarkCreationEvent.OnInit(
                        ...
                        toastMessages = linkmarkToastMessages,
                )
        )
    }
        ...
        LazyColumn {
            ...
            item {
                            ...
                            if (state.isInEditMode.not()) {
                                AnimatedVisibility(visible = state.hasApplyableMetadata) {
                                    TextButton(
                                            ...
                                            onClick = {
                                                vm.onEvent(
                                                        LinkmarkCreationEvent.OnApplyFromMetadata
                                                )
                                            }
                                    ) {
                ...
                Spacer(
                        modifier =
                                Modifier.height(
                                        if (state.isInEditMode.not() && state.hasApplyableMetadata)
                                                0.dp
                                        else 12.dp
                                )
                )
            ...
            item {
                BookmarkInfoContent(
                        ...
                        onChangeDescription = { newDescription ->
                            vm.onEvent(
                                    LinkmarkCreationEvent.OnChangeDescription(
                        ...
                        showInfoLabel = false,
                        onClearLabel = { vm.onEvent(LinkmarkCreationEvent.OnClearLabel) },
            ...
            item {
                BookmarkPinToggleRow(
                        ...
                        enabled = state.isLoading.not(),
                        onClick = { vm.onEvent(LinkmarkCreationEvent.OnTogglePinned) },
                )
            }
            item {
