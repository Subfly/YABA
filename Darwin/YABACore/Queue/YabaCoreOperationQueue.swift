//
//  YabaCoreOperationQueue.swift
//  YABACore
//
//  FIFO serialized persistence work, analogous to Compose `CoreOperationQueue`.
//  Each job receives its own `ModelContext` from `YabaCoreStore` and must leave the store
...
public final class YabaCoreOperationQueue: @unchecked Sendable {
    public static let shared = YabaCoreOperationQueue()
    ...
    public func queue(
        ...
        queue.async { [weak self] in
            ...
            do {
                let context = try YabaCoreStore.makeWriteContext()
                try operation(context)
                try YabaCoreStore.save(context)
                completion?(nil)
            } catch {
                completion?(error)
            }
