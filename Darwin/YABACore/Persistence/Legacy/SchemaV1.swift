...
typealias LegacyBookmark = SchemaV1.Bookmark
typealias LegacyCollection = SchemaV1.Collection
typealias LegacyDataLog = SchemaV1.DataLog
...
enum SchemaV1: VersionedSchema {
    ...
    final class Bookmark {
        ...
        var bookmarkType: LegacyBookmarkType {
            LegacyBookmarkType(rawValue: type) ?? .none
        }
    ...
    final class Collection {
        ...
        var createdAt: Date = Date.now
        var editedAt: Date = Date.now
        var color: LegacyColor = LegacyColor.none
        var order: Int = -1 // Used only in custom ordering active
        var type: Int = 1
        var version: Int = 1

        var collectionType: LegacyCollectionType {
            LegacyCollectionType(rawValue: type) ?? .folder
        }
        ...
        init(
            ...
            type: LegacyCollectionType = .folder,
            version: Int = 1,
            parent: LegacyCollection? = nil,
            children: [LegacyCollection] = [],
            order: Int = -1
        ) {
    ...
    final class DataLog {
        ...
        var entityId: String = ""
        var entityType: LegacyEntityType = LegacyEntityType.bookmark
        var actionType: LegacyActionType = LegacyActionType.deleted
        var timestamp: Date = Date.now

        var fieldChangesJSON: String?
        var fieldChanges: [LegacyFieldChange]? {
            guard let fieldChangesJSON,
                  let data = fieldChangesJSON.data(using: .utf8)
            else { return nil }

            return try? JSONDecoder().decode([LegacyFieldChange].self, from: data)
        }

        init(
            logId: String = UUID().uuidString,
            entityId: String = "",
            entityType: LegacyEntityType = .bookmark,
            actionType: LegacyActionType = .deleted,
            timestamp: Date = .now,
            fieldChanges: [LegacyFieldChange]? = nil
        ) {
            self.logId = logId
