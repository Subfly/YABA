...
struct CanvmarkCreationContent: View {
    @Environment(\.dismiss)
    private var dismiss
    ...
    @State
    private var machine = CanvmarkCreationStateMachine()
    ...
    private var formList: some View {
        List {
            Section {
                previewContent(fallbackIcon: "pen-tool-02")
            } header: {
                previewHeader
            }
    ...
    private func bootstrap() async {
        ...
        {
            machine.replaceState(BookmarkFlowHydration.canvmarkUIState(from: bookmark))
            return
        }
        await machine.send(
            .onInit(
