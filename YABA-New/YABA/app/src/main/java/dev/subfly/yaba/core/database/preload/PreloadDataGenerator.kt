...
package dev.subfly.yaba.core.database.preload

import dev.subfly.yaba.core.database.DatabaseProvider
import dev.subfly.yaba.core.database.mappers.toFolderEntity
import dev.subfly.yaba.core.database.mappers.toTagEntity
import dev.subfly.yaba.core.database.preload.model.PreloadData
...
object PreloadDataGenerator {
    ...
    suspend fun seedDefaultData(rawJsonOverride: String? = null): Boolean {
        rawJsonOverride ?: return false

        val preloadData = decode(rawJsonOverride)
        val now = clock.now()
