...
/**
 * macOS-specific server implementation for device-to-device sync
...
actual class SyncServer(private val port: Int) {
    ...
    private fun getLocalIpAddress(): String? {
        var ifaddr: ifaddrs? = null
        getifaddrs(ifaddr.ptr)
        
        var current = ifaddr
        while (current != null) {
            val addr = current.pointed.ifa_addr
            if (addr != null) {
                val sockaddr = addr.reinterpret<sockaddr_in>()
                if (sockaddr.sin_family.toInt() == 2) { // AF_INET
                    val ip = inet_ntoa(sockaddr.sin_addr)?.toKString()
